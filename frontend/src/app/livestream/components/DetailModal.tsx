...
const DetailModal: React.FC<DetailModalProps> = ({
  ...
  const getPriceRange = () => {
    const filteredPrices = product.prices.filter((p) => {
      return (
        (!selectedCategory1 || p.category1 === selectedCategory1) &&
        (!selectedCategory2 ||
          p.category2 === selectedCategory2 ||
          !p.category2)
      );
    });
  ...
  const getExactPrice = (platform: string) => {
    if (selectedCategory1 === null && selectedCategory2 === null) {
      const priceForPlatform = product.prices.find(
  ...
  const platforms = Array.from(
    new Set(product.prices.map((price) => price.platform)),
  );
      ...
      >
        ...
        <Box sx={{ display: "flex", mb: 2 }}>
          ...
          <Box sx={{ display: "flex", flexDirection: "column", width: "50%" }}>
            ...
            <Typography variant="h6" sx={{ color: "#08D2ED" }}>
              {getPriceRange()}
            </Typography>
            {hasCategories && (
              <Box
                        ...
                        <Button
                          ...
                          onClick={() =>
                            category === product.categories[0]
                              ? handleCategory1Click(value)
                              : handleCategory2Click(value)
                          }
                        >
                          {value}
                        </Button>
