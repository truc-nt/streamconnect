...
type IBaseRepository[T interface{}] interface {
	queryRow(db qrm.Queryable, stmt postgres.Statement) (*T, error)
	queryRows(db qrm.Queryable, stmt postgres.Statement) ([]*T, error)
...
func (r *BaseRepository[T]) queryRow(db qrm.Queryable, stmt postgres.Statement) (*T, error) {
	var data T
...
func (r *BaseRepository[T]) queryRows(db qrm.Queryable, stmt postgres.Statement) ([]*T, error) {
	var data []*T
	err := stmt.Query(db, &data)
