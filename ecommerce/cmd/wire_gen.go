...
func initServer() server.IServer {
	...
	iUserRepository := repository.NewUserRepository(postgresqlDatabase)
	iUserAddressRepository := repository.NewUserAddressRepository(postgresqlDatabase)
	iUserService := service.NewUserService(iUserRepository, iUserAddressRepository)
	iUserAddressService := service.NewUserAddressService(iUserAddressRepository)
	iUserHandler := handler.NewUserHandler(iUserService, iUserAddressService)
	...
	iAclRoleUserRepository := repository.NewAclRoleUserRepository(postgresqlDatabase)
	iShopService := service.NewShopService(iShopRepository, iCartRepository, iAclRoleUserRepository)
	...
	iLivestreamProductService := service.NewLivestreamProductService(iLivestreamProductRepository, iLivestreamExternalVariantRepository, iLivestreamProductFollowerRepository, v)
	...
	iVoucherRepository := repository.NewVoucherRepository(postgresqlDatabase)
	...
	iOrderService := service.NewOrderService(iOrderRepository, iVoucherRepository, iOrderItemRepository, iOrderItemLivestreamExternalVariantRepository, iCartItemRepository, iExternalOrderRepository, iUserRepository, iUserAddressRepository, iLivestreamExternalVariantRepository, v)
	iOrderHandler := handler.NewOrderHandler(iOrderService)
	iVoucherUserRepository := repository.NewVoucherUserRepository(postgresqlDatabase)
	iVoucherService := service.NewVoucherService(iVoucherRepository, iVoucherUserRepository)
	iVoucherHandler := handler.NewVoucherHandler(iVoucherService)
	handlers := handler.ProvideHandlers(iUserHandler, iShopHandler, iProductHandler, iVariantHandler, iShopifyHandler, iExternalShopHandler, iExternalVariantHandler, iLivestreamHandler, iLivestreamProductHandler, iCartHandler, iOrderHandler, iVoucherHandler)
